...
setupVaultSecretsOperator() {
  ...
  cat <<EOF | vault policy write vault-external-secrets -
  path "secrets/data/*" {
    capabilities = ["read"]
  }
EOF
  ...
  vault write auth/kubernetes/role/vault-external-secrets \
    ...
    policies=vault-external-secrets \
    ttl=24h
}
...
#   kvault "velero/velero/velero-helm-values.txt"
  kvault "media-servers/plex/plex-helm-values.txt"
}
...
export VAULT_ADDR='http://192.168.16.30:8200'
